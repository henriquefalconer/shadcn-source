...
//
// Pipeline (mirrors /Users/vm/shadcn/_work/{fetch,extract,flatten,reapply-rewrites,gen-notices}.mjs):
...
//
// Defaults OUT to _work/bootstrap/tmp-registry (never writes to the real registry/).
...
const ARCHIVE = path.resolve(BOOTSTRAP, "..", "..") // /Users/vm/shadcn
const WORK = path.join(ARCHIVE, "_work") // read-only inputs: assets-report.json, extra-rewrites.json

const OUT = path.resolve(process.env.OUT || process.argv[2] || path.join(BOOTSTRAP, "tmp-registry"))
const RAW = path.join(BOOTSTRAP, "raw")
const ENUM = path.join(BOOTSTRAP, "enum")
const MANIFEST = path.join(BOOTSTRAP, "manifest.jsonl")

// Guardrail: never let a misconfigured OUT touch the real archive registry.
if (OUT === path.join(ARCHIVE, "registry")) {
  console.error(`refusing to write to ${OUT} -- this is the read-only reference tree`)
  process.exit(1)
}
...
async function enumerate() {
  fs.mkdirSync(ENUM, { recursive: true })
  const names = {}
  for (const [reg, scope] of Object.entries(SCOPES)) {
    // Capturing stdout through a pipe (execFile's default) silently truncates
    // large output at 64KiB -- the CLI's process appears to exit before the
    // OS pipe buffer fully drains. Redirecting to a real file sidesteps it.
    const tmpFile = path.join(os.tmpdir(), `shadcn-search-${reg}-${process.pid}.json`)
    const MAX_ATTEMPTS = 5
    let json
    for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
      const fd = fs.openSync(tmpFile, "w")
      try {
        execFileSync("npx", ["--yes", "shadcn", "search", scope, "-l", "5000", "--json"], {
          cwd: ARCHIVE,
        ...
        console.log(`  ${reg}: search failed (attempt ${attempt + 1}/${MAX_ATTEMPTS}), retrying in ${backoff / 1000}s`)
    ...
    fs.writeFileSync(path.join(ENUM, `${reg}.json`), JSON.stringify(json, null, 2))
    names[reg] = json.items.map((i) => i.name)
    console.log(`  ${reg}: ${names[reg].length} items`)
  }
  return names
}
